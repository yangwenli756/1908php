...
<form  action="{{url('/goods/update',$res->goods_id)}}" method="post" class="form-horizontal" role="form" enctype="multipart/form-data">
	...
	<div class="form-group">
		...
		<div class="col-sm-4">
			<select name="cate_id">
				...
				@foreach($data as $k=>$v)
					<option value="{{$v->cate_id}}" @if($v->cate_id==$res->cate_id) selected="selected" @endif>{{str_repeat('|-',$v->level*2)}}{{$v->cate_name}}</option>
				@endforeach
			</select>
	...
	<div class="form-group">
		...
		<div class="col-sm-4">
			<textarea name="goods_desc">{{$v->goods_desc}}</textarea>
		</div>
	</div>
	<div class="form-group">
		<label for="firstname" class="col-sm-2 control-label">上传文件</label>
		<div class="col-sm-8">
			<img src="{{env('UPLOAD_URL')}}{{$res->goods_img}}" width="50px">
	...
	<div class="form-group">
		...
		<div class="col-sm-4">
			<input type="text" class="form-control" value="{{$res->goods_score}}" name="goods_score" id="firstname"
				   placeholder="请输入商品货号">
	...
	<div class="form-group">
		...
		<div class="col-sm-4">
			<input type="text" class="form-control" value="{{$res->goods_price}}" name="goods_price" id="firstname"
				   placeholder="请输入商品价格">
	...
	<div class="form-group">
		...
		<div class="col-sm-4">
			<input type="text" class="form-control" value="{{$res->goods_num}}" name="goods_num" id="firstname"
				   placeholder="请输入商品库存">
			<b style="color:red;">{{$errors->first('goods_num')}}</b>
		</div>
	</div>
	<div class="form-group">
		...
		<div class="radio">
			<label>
				<input type="radio" name="is_best" id="optionsRadios1" value="1" @if($res->is_best==1)checked @endif>是<br><br>
				<input type="radio" name="is_best" id="optionsRadios1" value="2" @if($res->is_best==2)checked @endif>否
	...
	<div class="form-group">
		...
		<div class="radio">
			<label>
				<input type="radio" name="is_hot" id="optionsRadios1" value="1" @if($res->is_hot==1)checked @endif>是<br><br>
				<input type="radio" name="is_hot" id="optionsRadios1" value="2" @if($res->is_hot==2)checked @endif>否
	...
	<div class="form-group">
		<div class="col-sm-offset-2 col-sm-10">
			<button type="submit" class="btn btn-default add">修改</button>
		</div>
	</div>
</form>
